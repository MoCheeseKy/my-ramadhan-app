...
function AyatCard({
  ...
  const renderArabic = () => {
    ...
    if (lastIdx < ayat.teksArab.length) {
      parts.push(<span key='last'>{ayat.teksArab.slice(lastIdx)}</span>);
    }
      ...
      <div className='flex items-center justify-between px-5 py-3 border-b border-slate-50'>
        <div
          className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-black ${
            isLastRead ? 'bg-[#1e3a8a] text-white' : 'bg-blue-50 text-[#1e3a8a]'
          }`}
        >
          {ayat.nomorAyat}
        </div>
  ...
  return (
    ...
    >
      ...
      <main className='max-w-md mx-auto p-5 space-y-4'>
        {/* Surah header card */}
        {!loading && surah && (
          <div className='bg-gradient-to-br from-[#1e3a8a] to-[#312e81] rounded-3xl p-6 text-white text-center relative overflow-hidden mb-2'>
            ...
            <p className='text-indigo-300 text-sm mt-1'>{surah.arti}</p>
