...
export default function TrackerKalender() {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    // Safety check just in case string is not valid JSON
    let localUser = null;
    try {
      localUser = JSON.parse(localUserStr);
    } catch (e) {
      console.error('Local user data corrupt');
    }

    if (!localUser || !localUser.personal_code) {
      setLoading(false);
      return;
    }
  ...
  const handleDeleteCustomHabit = async (id) => {
    if (!userId || !window.confirm('Hapus target tambahan ini?')) return;
